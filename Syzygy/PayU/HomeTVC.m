...
- (void)viewDidLoad {
    ...
    self.arrPaymentParamValue = [NSMutableArray arrayWithObjects:[self.arrMerchantKey objectAtIndex:envIndex],[self.arrMerchantID objectAtIndex:envIndex],[Utils getTxnID],[NSString stringWithFormat:@"%.2f",[[self.package objectForKey:@"payable"]doubleValue]],@"9717410858",@"umangarya336@gmail.com",[self.package objectForKey:@"full_name"],@"http://www.google.com",@"http://www.google.com",@"",@"udf1",@"udf2",@"udf3",@"udf4",@"udf5",@"udf6",@"udf7",@"udf8",@"udf9",@"udf10",[self.arrEnvironment objectAtIndex:envIndex], nil];
...
- (IBAction)btnTappedChangeEnv:(id)sender {
   /* [[notification name]isEqualToString:@"paymentSuccess"]){
        self.navigationController.navigationBar.hidden = NO;
        [finishVIew removeFromSuperview];
    }else if ([[notification name]isEqualToString:@"paymentBack"]){*/
    [[NSNotificationCenter defaultCenter]postNotificationName:@"paymentBack" object:nil];
